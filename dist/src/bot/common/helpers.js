...
const checkoutMessage = (offer, code) => {
    ...
    return i18n_1.default.t(code, 'orderCheckout') +
        i18n_1.default.t(code, 'orderCheckoutFeePayer') + ': ' + i18n_1.default.t(code, offer.feePayer) + '\n' +
        i18n_1.default.t(code, 'orderValue') + ': ' + offer.offerValue + ' p\n' +
        i18n_1.default.t(code, 'orderCheckoutPaymentMethod') + ': ' + i18n_1.default.t(code, paymentMethod.method) + '\n' +
        i18n_1.default.t(code, 'orderCheckoutFee') + ': ' + offer.feeBaked + ' p\n' +
        i18n_1.default.t(code, 'orderCheckoutEstimatedShipping') + ': ' + new Date(offer.estimatedShipping).toLocaleDateString() + '\n' +
        i18n_1.default.t(code, 'orderCheckoutProductDetails') + ': ' + offer.productDetails + '\n' +
        i18n_1.default.t(code, 'orderCheckoutProductAdditionalDetails') + ': ' + (offer.productAdditionalDetails || i18n_1.default.t(code, 'notDefined')) + '\n' +
        i18n_1.default.t(code, 'orderCheckoutRestDetails') + ': ' + (offer.restDetails || i18n_1.default.t(code, 'notDefined')) + '\n' +
        i18n_1.default.t(code, 'orderCheckoutRefundDetails') + ': ' + offer.refundDetails + '\n' +
        i18n_1.default.t(code, 'orderCheckoutSellerWalletData') + ': ' + (offer.sellerWalletData || i18n_1.default.t(code, 'notDefined')) + '\n' +
        i18n_1.default.t(code, 'orderCheckoutOfferStatus') + ': ' + offer.offerStatus + '\n' +
        i18n_1.default.t(code, 'buyer') + ': ' + (offer.role === 'buyer' ? offer.initiator_chatId : offer.partner_chatId) + '\n' +
        i18n_1.default.t(code, 'seller') + ': ' + (offer.role === 'seller' ? offer.initiator_chatId : offer.partner_chatId);
};
...
const checkoutArbMessage = function (arb, code) {
    return i18n_1.default.t(code, 'arbitraryCheckout') +
        i18n_1.default.t(code, 'arbitraryReason') + ': ' + arb.reason + '\n' +
        i18n_1.default.t(code, 'arbitraryStatus') + ': ' + i18n_1.default.t(code, arb.status) + '\n' +
        i18n_1.default.t(code, 'arbitraryComment') + ': ' + arb.comment + '\n' +
        ...
        i18n_1.default.t(code, 'arbitraryInitiator') + ': ' + (arb.offer.initiator.username || arb.offer.initiator.chatId) + '\n';
};
...
const getInvoiceValue = (invoices) => {
    return invoices.toArray().filter(i => i.type == Invoices_1.InvoicesType.IN).reduce((s, c) => s + c.value, 0);
};
exports.getInvoiceValue = getInvoiceValue;
const getInvoiceFee = (invoices) => {
    return invoices.toArray().filter(i => i.type == Invoices_1.InvoicesType.IN).reduce((s, c) => s + c.fee, 0);
};
...
const getArbMessage = (arb, chatId, code) => {
    const message = arb.reduce((sum, cur) => {
        ...
        const offerValue = cur.offer.invoices.toArray().filter(i => i.type == Invoices_1.InvoicesType.IN).reduce((s, c) => s + c.value, 0);
        sum += `ID ${cur.id}: ${i18n_1.default.t(code, 'arbitraryOfferId', { id: cur.offer.id })}, ${(0, exports.getOppositeUser)(cur.offer, chatId).chatId} - ${offerValue} ${currency.match(/QIWI|CARD/) ? 'RUB' : currency}, ${i18n_1.default.t(code, cur.status)}\n`;
        return sum;
