...
export class globalComposer extends BaseComposer {
  ...
  activeArbitraries = async (ctx: BotContext) => {
  ...
  allArbitraries = async (ctx: BotContext) => {
  ...
  web = async (ctx: BotContext) => {
    ...
    if (user.password) {
      ...
      await ctx.reply(
        ctx.i18n.t('accountWebLink', { link: `${url}/#login?p=${password}&l=${login}` }) +
          '\n' +
          ctx.i18n.t('accountWebCreds', { pass: password, login: login }),
        { parse_mode: 'HTML' },
      );
    }
  };
  ...
  @Hears('rules')
  rules = async (ctx: BotContext) => await ctx.reply(ctx.i18n.t('rules'));

  @Hears('instructions')
  instructions = async (ctx: BotContext) => await ctx.reply(ctx.i18n.t('instructions'));

  @Hears('info')
  info = async (ctx: BotContext) => await ctx.reply(ctx.i18n.t('info'));

  @Hears('findUser')
  findUser = async (ctx: BotContext) =>
    await ctx.reply(ctx.i18n.t('findUserInfo'), { reply_markup: findUserMenu(ctx) });

  @Hears('changeLang')
  changeLang = async (ctx: BotContext) =>
    await ctx.reply(ctx.i18n.t('languageGroup'), { reply_markup: languageMenu(ctx) });

  @On('inline_query')
  inline_query = async (ctx: BotContext) => {
