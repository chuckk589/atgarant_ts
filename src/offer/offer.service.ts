...
export class OfferService {
  async update(id: number, updateOfferDto: UpdateOfferDto) {
    return await this.em.nativeUpdate(Offers, {
      id: id
    }, {
  ...
  async offerAction(id: string): Promise<Offerstatuses> {
    const offer = await this.em.findOneOrFail(Offers, { id: Number(id) }, { populate: ['partner', 'initiator', 'offerStatus'] })
  ...
  async findAll(userId?: string) {
    const options = userId
      ...
      : {}
    return await this.em.find(Offers, options, { populate: ['initiator', 'partner', 'offerStatus', 'paymentMethod', 'reviews'] })
  }
}
