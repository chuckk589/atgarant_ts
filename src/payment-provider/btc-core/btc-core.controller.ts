...
export class BtcCoreController extends BasePaymentController {
  ...
  worker = async () => {
    const invoices = await this.btcCoreService.fetchMatchingInvoices(
      this.options.incomingTimeout,
      this.options.outgoingTimeout,
    );
  ...
  sellerWithdraw = async (offer: Offers) => {
    const sellerPayout =
      getInvoiceValue(offer.invoices) +
      getInvoiceFee(offer.invoices) * (offer.feePayer == OffersFeePayer.BUYER ? 0 : -1);
  ...
  withDraw = async (amount: number, address: string, type: InvoicesType, offerId: number, currency = 'BTC') => {
