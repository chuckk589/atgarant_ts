...
export class AppConfigService {
  constructor(private readonly configService: ConfigService, @Inject('test') any: any) {}
  ...
  get payments(): PM[] {
    ...
    return operationMode == 'btc-core'
      ? keys.map((k) => new PM(k.split('_').pop(), process.env[k])).filter((p) => p.method == 'BTC')
      : keys.map((k) => new PM(k.split('_').pop(), process.env[k]));
  }
