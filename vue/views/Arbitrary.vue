<template>
  <div>
    ...
    >
      <template #slider>
        <CInputCheckbox
          @click="handleSidesWallets"
          :checked.sync="modalConfig.custom.customPayout"
          label="Распределить выплату"
        />
        <CCollapse :show="modalConfig.custom.customPayout">
          <CInput
            class="range"
            :label="'Покупатель ' + modalConfig.custom.sliderValBuyer + '%'"
            type="range"
            min="0"
            max="100"
            :value.sync="modalConfig.custom.sliderValBuyer"
            @input="rangeI($event, 0)"
          ></CInput>
          <CInput
            class="range"
            :label="'Продавец ' + modalConfig.custom.sliderValSeller + '%'"
            type="range"
            min="0"
            max="100"
            :value.sync="modalConfig.custom.sliderValSeller"
            @input="rangeI($event, 1)"
          ></CInput>
        </CCollapse>
      </template>
    </CExtendedModal>
    <CDataTable :items="arbs" :fields="fields" sorter columnFilter>
      <template #offer="{ item }"
        ><td data-label="ID сделки">
          {{ item.offer.id }}
        </td></template
      >
      <template #arbiter="{ item }"
        ><td data-label="Арбитр">
          {{ item.arbiter.username || item.arbiter.chat_id }}
        </td></template
      >
      <template #initiator="{ item }"
        ><td data-label="Инициатор арбитража">
          {{ item.initiator.username || item.initiator.chat_id }}
        </td></template
      >
      <template #status="{ item }"
        ><td data-label="Статус арбитража">
        ...
        <td>
          <CDropdown toggler-text="Действия" class="m-2" color="secondary">
            <CDropdownItem
              ...
              >Вынести решение</CDropdownItem
            >
            <CDropdownItem
              @click="openDispute(item)"
              v-show="!isArb"
              :disabled="isDisputed(item)"
              >Обжаловать</CDropdownItem
            >
            <CDropdownItem @click="getChatHistory(item)"
              >История чата</CDropdownItem
            >
          </CDropdown>
        </td>
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "offer", label: "ID сделки" },
        { key: "chatId", label: "ID чата арбитража" },
  ...
  mounted: function () {
    this.$http({
      method: "GET",
      url: `/v1/arbitrary${
        this.userData.role === 1 || this.userData.role === 2
          ? "/"
  ...
  methods: {
    ...
    isDisputed(item) {
      return !(
        item.status == "closed" &&
        (new Date() - new Date(item.updatedAt)) / (1000 * 60 * 60) <= 12
      );
    },
    closeArb(item) {
    ...
    handleSidesWallets(a) {
      if (
        !this.modalConfig.data.cur.offer.sellerWalletData ||
        !this.modalConfig.data.cur.offer.buyerWalletData
      ) {
        a.preventDefault();
        alert(
          "Для разрешения арбитража с распределением выплат требуется чтобы у обоих сторон сделки был указан кошелек для выплат"
        );
      }
    },
    closeModalHandler() {
      if (this.modalConfig.data.type == "closeArbitrary") {
        ...
        const comment = this.modalConfig.data.fields.find(
          (f) => f.label == "Комментарий"
        ).value;
        ...
        this.$http({
          method: "POST",
          url: `/v1/arbitrary/${this.modalConfig.data.cur.id}/close/`,
          data: {
            comment: comment,
            ...customPayout,
          },
    ...
    getChatHistory(item) {
      this.$http({
        method: "GET",
        url: `/v1/arbitrary/${item.id}/history/`,
      }).then((res) => {
        const blob = new Blob([res.data], { type: "text/plain;charset=utf-8" });
        FileSaver.saveAs(
          blob,
          `${item.id}_${new Date().toLocaleDateString()}.txt`
        );
      });
    },
    openDispute(item) {
      this.$http({
        method: "POST",
        url: `/v1/arbitrary/${item.id}/dispute/`,
        data: { initiator: this.userData.id },
      }).then((r) => {
        item.status = r.data.newStatus;
      });
    },
  },
};
