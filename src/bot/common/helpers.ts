...
export const checkoutArbMessage = function (arb: Arbitraries, code: string) {
    return i18n.t(code, 'arbitraryCheckout') +
        i18n.t(code, 'arbitraryReason') + ': ' + arb.reason + '\n' +
        i18n.t(code, 'arbitraryStatus') + ': ' + i18n.t(code, arb.status) + '\n' +
        i18n.t(code, 'arbitraryComment') + ': ' + arb.comment + '\n' +
        i18n.t(code, 'arbitraryPayout', { buyer: arb.buyerPayout, seller: arb.sellerPayout }) + '\n' +
        i18n.t(code, 'arbitraryOfferId', { id: arb.offer.id }) + '\n' +
        i18n.t(code, 'arbitraryInitiator') + ': ' + (arb.offer.initiator.username || arb.offer.initiator.chatId) + '\n'
}
...
export const getInvoiceValue = (invoices: Collection<Invoices>): number => {
    return invoices.toArray().filter(i => i.type == InvoicesType.IN).reduce((s, c) => s + c.value, 0)
}
export const getInvoiceFee = (invoices: Collection<Invoices>): number => {
    return invoices.toArray().filter(i => i.type == InvoicesType.IN).reduce((s, c) => s + c.fee, 0)
}
/**
...
export const getArbMessage = (arb: Arbitraries[], chatId: number, code: string): string => {
    const message = arb.reduce((sum, cur) => {
        ...
        const offerValue = cur.offer.invoices.toArray().filter(i => i.type == InvoicesType.IN).reduce((s, c) => s + c.value, 0)
        sum += `ID ${cur.id}: ${i18n.t(code, 'arbitraryOfferId', { id: cur.offer.id })}, ${getOppositeUser(cur.offer, chatId).chatId} - ${offerValue} ${currency.match(/QIWI|CARD/) ? 'RUB' : currency}, ${i18n.t(code, cur.status)}\n`
        return sum
